...
== Paso 1: descarga el archivo ISO u OVA e implementa la máquina virtual
...
== Paso 2: crea el nodo de gestión admin y configura la red
...
== Paso 3: Configura la sincronización horaria
...
.. Añade una nueva línea por cada servidor de hora predeterminado que quieras añadir. Los servidores de hora predeterminados deben ser los mismos servidores NTP que se usan en el clúster de almacenamiento que vas a usar en un link:task_mnode_install.html#step-4-set-up-the-management-node["paso posterior"].
+
[listing]
----
vi /etc/ntp.conf
...
NOTE: No modifiques el NTP una vez completada la configuración de la sincronización horaria, ya que esto afecta al NTP cuando ejecutas el link:task_mnode_install.html#step-4-set-up-the-management-node["comando setup"] en el nodo de gestión.



[[step-4-set-up-the-management-node]]
== Paso 4: Configura el nodo de gestión
