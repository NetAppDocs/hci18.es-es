...
A continuación se ofrece una descripción general de los pasos del procedimiento:
...
. Ejecute `show version` el comando. Consulte el siguiente ejemplo:
+
[listing]
----
...
----
. Si las versiones no coinciden, debes actualizar el sistema operativo. Consulta el enlace: Consulta el link:https://community.mellanox.com/s/article/howto-upgrade-switch-os-software-on-mellanox-switch-systems["Guía de actualización de software de Mellanox"^] para obtener más detalles.
...
Realiza los pasos para configurar el switch de reemplazo. Consulta el enlace: consulta link:https://docs.mellanox.com/display/MLNXOSv381000/Configuration+Management["Gestión de la configuración de Mellanox"^] para más detalles.
...
----
.  `yes`Escriba para confirmar el reinicio.
