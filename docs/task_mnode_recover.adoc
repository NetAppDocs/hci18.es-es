...
NOTE: No modifiques el NTP una vez completada la configuración de la sincronización horaria, ya que esto afecta al NTP cuando ejecutas el <<step-6,comando re-deploy>> en el nodo de gestión.
...
. [[step-6]] Configura y ejecuta el comando de reimplementación del nodo de gestión para conectarte a los volúmenes persistentes alojados en el clúster e iniciar los servicios con los datos de configuración anteriores del nodo de gestión:
+
